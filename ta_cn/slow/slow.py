...
def ATR_TA(high, low, close, timeperiod=14):
    """ATR真实波幅N日平均
...
def DM(high, low, timeperiod=14):
    ...
    HD[np.isnan(HD) & (~np.isnan(high))] = 0
    LD[np.isnan(LD) & (~np.isnan(low))] = 0
...
def DI(high, low, close, timeperiod=14):
    ...
    # 数据有效区开始，值由nan要设成0
    tr[np.isnan(tr) & (~np.isnan(close))] = 0
...
def _SLOPE(S, N=14):
    return np_to_pd(S).rolling(N).apply(lambda x: np.polyfit(range(N), x, deg=1)[0], raw=True)


def _FORCAST(S, N=14):
    return np_to_pd(S).rolling(N).apply(lambda x: np.polyval(np.polyfit(range(N), x, deg=1), N - 1), raw=True)
